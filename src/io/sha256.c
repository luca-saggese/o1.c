...
static void sha256_block(hd_sha256_ctx *c, const uint8_t *p) {
    uint32_t w[64];
    for (int i = 0; i < 16; i++) {
...
void hd_sha256_final(hd_sha256_ctx *c, uint8_t out[32]) {
    ...
    uint8_t pad = 0x80;
    hd_sha256_update(c, &pad, 1);
    uint8_t zero = 0x00;
    while (c->buflen != 56) hd_sha256_update(c, &zero, 1);

    uint8_t lenbuf[8];
    for (int i = 0; i < 8; i++) lenbuf[i] = (uint8_t)(bitlen >> (56 - i * 8));
    /* hd_sha256_update would corrupt bitlen, so write the block directly. */
