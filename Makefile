...
$(TIMING_OBJDIR)/%.o: %.c
	@mkdir -p $(dir $@)
	$(CC) $(CFLAGS) -DO1_DEBUG_TIMING $(CPPFLAGS) $(CUDA_CPPFLAGS) -MMD -MP -c -o $@ $<
...
$(TIMING_BLOCK_OBJDIR)/%.o: %.c
	@mkdir -p $(dir $@)
	$(CC) $(CFLAGS) -DO1_DEBUG_TIMING -DO1_DEBUG_BLOCK_TIMING $(CPPFLAGS) $(CUDA_CPPFLAGS) -MMD -MP -c -o $@ $<
...
$(CUBIN)/%.o: src/cuda/%.cu
	@mkdir -p $(CUBIN)
	$(NVCC) -arch=sm_121 -O2 -std=c++17 $(CPPFLAGS) $(CUDA_CPPFLAGS) -MMD -MP -c -o $@ $<
...
# Header dependencies generated by -MMD. Without these, editing a header
# (e.g. a struct layout change in forward.h) leaves stale objects that link
# silently against the new layout and corrupt the ABI at runtime.
-include $(SRCS:.c=.d)
-include $(wildcard $(CUBIN)/*.d)
-include $(wildcard src/*.d src/*/*.d src/*/*/*.d tests/*/*.d tests/*.d)
