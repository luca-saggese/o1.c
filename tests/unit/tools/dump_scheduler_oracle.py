...
into a flat text file that the C unit test parses.

Usage:  python dump_scheduler_oracle.py <out.txt> [--steps N]
"""
import os
import sys
...
# A fixed synthetic example: length-N vectors
N = 64


def make_example():
    z = torch.randn(1, N, dtype=torch.float32)
    mo = torch.randn(1, N, dtype=torch.float32) * 0.3
    noise = torch.randn(1, N, dtype=torch.float32)
    return z, mo, noise
...
def main():
    ...
    lines = []
    lines.append("# M1-post scheduler oracle dump")
    ...
    for name, (steps, shift, sched_name, tlist) in RECIPES.items():
        ...
        sigmas = sched.sigmas  # torch tensor, already includes terminal 0.0
        ...
        lines.append("SIGMAS " + " ".join(repr(v) for v in sig_list))
        lines.append("NOISESCALE " + " ".join(repr(v) for v in nss))

        # step outputs on a synthetic example
        z, mo, noise = make_example()

        if sched_name == "flash":
            # Euler step with s_noise from schedule[0], noise_clip_std=8.0
            s_noise = nss[0]
            ncs = NOISE_SCALE
            out = sched.step(
                mo, torch.tensor(float(sched.timesteps[0]), dtype=torch.float32),
                z, s_noise=s_noise, noise_clip_std=ncs, return_dict=False)[0]
