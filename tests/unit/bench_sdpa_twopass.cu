...
int main(void) {
    printf("=== C4 two-pass SDPA A/B (SEQLEN=%d image=%d text=%d) ===\n",
           SEQLEN, IMAGE_LEN, TEXT_LEN);
    ...
    if (seq.S != SEQLEN) { printf("FAIL: expected SEQLEN=%d\n", SEQLEN); return 1; }

    /* ar_len per the C4 spec: text_len - 1 */
    int ar_len = seq.text_len - 1;
    printf("ar_len (text_len-1) = %d ; gen_len = %d\n", ar_len, SEQLEN - ar_len);

    /* ---- device operands ---- */
    size_t q_bytes = (size_t)NH * SEQLEN * HD * 2;
    size_t kv_bytes = (size_t)NKV * SEQLEN * HD * 2;
    uint16_t *qh = (uint16_t *)malloc(q_bytes);
    uint16_t *kh = (uint16_t *)malloc(kv_bytes);
    uint16_t *vh = (uint16_t *)malloc(kv_bytes);
    fill_bf16(qh, (size_t)NH * SEQLEN * HD, 1);
    fill_bf16(kh, (size_t)NKV * SEQLEN * HD, 2);
    fill_bf16(vh, (size_t)NKV * SEQLEN * HD, 3);
    ...
    void *maskd = dev_alloc((size_t)SEQLEN * SEQLEN * 2);
    cudaMemcpy(maskd, seq.mask_bf16, (size_t)SEQLEN * SEQLEN * 2, cudaMemcpyHostToDevice);

    void *out_ref = dev_alloc(q_bytes);
    void *out_two = dev_alloc(q_bytes);
    float *fr = (float *)malloc((size_t)NH * SEQLEN * HD * 4);
    float *ft = (float *)malloc((size_t)NH * SEQLEN * HD * 4);
    ...
    int rc = hd_sdpa_create(&p_ref, 1, NH, NKV, SEQLEN, SEQLEN, HD, scale);
    if (rc != 0) { printf("FAIL: hd_sdpa_create: %s\n", hd_cuda_errbuf()); return 1; }
    double t_ref_create = now_s() - t0;

    t0 = now_s();
    hd_sdpa_plan *p_two = NULL;
    rc = hd_sdpa_create_split(&p_two, 1, NH, NKV, SEQLEN, ar_len, HD, scale);
    ...
    hd_bf16_buf_to_f32(fr, fr, (size_t)NH * SEQLEN * HD);
    hd_bf16_buf_to_f32(ft, ft, (size_t)NH * SEQLEN * HD);

    double max_abs, nrmse, cos;
    stats(fr, ft, (size_t)NH * SEQLEN * HD, &max_abs, &nrmse, &cos);
    ...
    for (int h = 0; h < NH; h++) {
        ...
        stats(fr + head_off + (size_t)ar_len * HD, ft + head_off + (size_t)ar_len * HD,
              (size_t)(SEQLEN - ar_len) * HD, &ma_gen, &nr_gen, &cs_gen);
        if (h == 0)
    ...
    double flops_full = 2.0 * 2.0 * (double)NH * SEQLEN * SEQLEN * HD;  /* qk + av */
    printf("  mixed effective         : %.2f TFLOP/s\n",
           flops_full / (ms_ref * 1e-3) / 1e12);
    double flops_two = 2.0 * 2.0 * (double)NH *
        ((double)ar_len * ar_len + (double)(SEQLEN - ar_len) * SEQLEN) * HD;
